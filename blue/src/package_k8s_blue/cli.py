...
from .workflow import k8s_workflow

USAGE = ("Usage: blue <build|create|delete> "
         "[-f|--file colors.yml] [--dry-run]\n"
         "       blue kubectl <kubectl arguments>\n"
         "\n"
...
async def run(*args):
    ...
    if command in LIFECYCLE:
        return await run_cli(k8s_workflow, default_args(args))
    if command == "kubectl":
        return operator.run(_find(), "kubectl", args[1:])

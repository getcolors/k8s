...
def host_alias(opts: dict) -> str:
    ...
    return opts.get("profile") or "k8s"
...
# migration is in flight; see `owned_markers`.
def superseded_begin_marker(alias: str) -> str:
    return f"# BEGIN k8s {alias} ANSIBLE MANAGED BLOCK"


def superseded_end_marker(alias: str) -> str:
    return f"# END k8s {alias} ANSIBLE MANAGED BLOCK"
...
def owned_markers(alias: str) -> dict:
    ...
    return {"begin": {begin_marker(alias), superseded_begin_marker(alias)},
            "end": {end_marker(alias), superseded_end_marker(alias)}}

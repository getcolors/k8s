...
export function hostAlias(opts: Opts): string {
  return String(opts.profile || "k8s");
}
...
// The marker this package wrote before it adopted the standard: the package
// name prepended to the alias. Superseded, and recognised as ours while the
// migration is in flight; see `ownedMarkers`.
export function supersededBeginMarker(alias: string): string {
  return `# BEGIN k8s ${alias} ANSIBLE MANAGED BLOCK`;
}

export function supersededEndMarker(alias: string): string {
  return `# END k8s ${alias} ANSIBLE MANAGED BLOCK`;
}
...
// ownership check below does not read k8s's own old block as a hand-written
...
export function ownedMarkers(alias: string): { begin: Set<string>; end: Set<string> } {
  return {
    begin: new Set([beginMarker(alias), supersededBeginMarker(alias)]),
    end: new Set([endMarker(alias), supersededEndMarker(alias)]),
  };
}

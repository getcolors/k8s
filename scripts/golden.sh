...
checks() {
  ...
  grep -q "dig +short A hello.fixture.example" "$delete" || { echo "golden: $profile: the delete does not wait for the DNS withdrawal" >&2; exit 1; }
  grep -q "dig +short NS fixture.example" "$delete"

(ns pin
  ...
  K8s's launcher pins only k8s. `green` comes transitively from k8s's
  deps.edn, so there is one package pin site."
  (:require
...
(def ^:private site
  "Where a copied payload records which k8s commit to resolve.
  ...
  {:path "skills/package-k8s-green/green"
   :rx #"\(def \^:private k8s-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate k8s-sha in " (:path site))}
          (let [[head err] (repo-head "." "k8s")]
            (cond
